...
function f_tableExists(mysqli $connection, $tablename, $database = false) {
    
    if ( !$database ) {
        
        $res = mysqli_query($connection, "SELECT_DATABASE()");
        
        $database = mysqli_result($res, 0);
    }
    
    $res = mysqli_query($connection, "select * from information_schema.tables
